            ...
            for case in cases:
                ...
                """,
                    (
                        ...
                        case.get("minutes", ""),
                        case.get("evidence", ""),
                        case.get("status", "Awaiting Evidence"),
                        case.get("list", "Checklist"),
                        (
                            case["assessment"]["assessed_by"]
                            if case.get("assessment")
                            else None
                        ),
                        (
                            case["assessment"]["assessment_date"]
                            if case.get("assessment")
                            else None
                        ),
                        (
            ...
            for resp in responsibilities:
                ...
                """,
                    (
                        ...
                        resp.get("name"),
                        resp.get("parent_id"),
                        resp.get("is_posting_level", False),
                        str(resp.get("contacts", [])),
                    ),
                )
    ...
    try:
        if os.path.exists(template_file):
            ...
            if isinstance(templates, list):
                ...
                print(
                    "Unexpected format in email_templates.json, skipping template migration."
                )
        else:
