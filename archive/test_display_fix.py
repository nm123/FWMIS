...
def test_display_stripping():
    ...
    try:
        ...
        cursor.execute("SELECT transaction_no, list, status FROM cases WHERE transaction_no LIKE '%-LS' OR transaction_no LIKE '%-WOR' ORDER BY transaction_no")
        ...
        for case in suffixed_cases:
            ...
            print(f"  DB: {transaction_no} -> Display: {display_no} (List: {list_name}, Status: {status})")
        ...
        for display_no in test_display_numbers:
            ...
            for case_no in possible_case_nos:
                cursor.execute("SELECT id, list, status FROM cases WHERE transaction_no = ?", (case_no,))
                result = cursor.fetchone()
                if result:
                    case_id, list_name, status = result
                    print(f"  Found: {case_no} -> ID: {case_id}, List: {list_name}, Status: {status}")
                    break
            else:
