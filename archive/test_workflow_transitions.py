...
def test_workflow_transitions():
    ...
    try:
        ...
        if test_case:
            ...
            print(f"Current: List={current_list}, Status={current_status}, FY_ID={current_fy_id}")

            # Test Confirmed status transition
            print("\n=== Testing Confirmed Status Transition ===")
            success = handle_case_status_change(case_id, transaction_no, "Confirmed", current_list)
            print(f"Workflow transition result: {success}")

            # Check the original case after transition (should be Confirmed in Checklist)
            cursor.execute("SELECT list, status, fy_id FROM cases WHERE id = ?", (case_id,))
            original_case = cursor.fetchone()
            print(f"Original case after transition: List={original_case[0]}, Status={original_case[1]}, FY_ID={original_case[2]}")

            # Check if copy exists in Lead Schedule
            cursor.execute("SELECT id, list, status, fy_id FROM cases WHERE transaction_no = ? AND list = 'Lead Schedule'", (f"{transaction_no}-LS",))
            copied_case = cursor.fetchone()
            if copied_case:
                print(f"Copied case in Lead Schedule: ID={copied_case[0]}, Transaction={transaction_no}-LS, List={copied_case[1]}, Status={copied_case[2]}, FY_ID={copied_case[3]}")
                copy_exists = True
            else:
            ...
            cursor.execute("SELECT COUNT(*) FROM financial_years WHERE id = ?", (original_case[2],))
            original_fy_exists = cursor.fetchone()[0]
            print(f"Original case FY_ID {original_case[2]} exists in financial_years: {original_fy_exists > 0}")

            if copy_exists:
                cursor.execute("SELECT COUNT(*) FROM financial_years WHERE id = ?", (copied_case[3],))
                copy_fy_exists = cursor.fetchone()[0]
                print(f"Copied case FY_ID {copied_case[3]} exists in financial_years: {copy_fy_exists > 0}")

            # Reset cases for next test
            cursor.execute("UPDATE cases SET list = ?, status = ?, is_finalized = 0 WHERE transaction_no = ?", ("Checklist", "Alleged", transaction_no))
            cursor.execute("DELETE FROM cases WHERE transaction_no = ?", (f"{transaction_no}-LS",))
            conn.commit()
            print("Reset cases for next test")

            # Test Valid status transition
            print("\n=== Testing Valid Status Transition ===")
            success = handle_case_status_change(case_id, transaction_no, "Valid", current_list)
            print(f"Workflow transition result: {success}")

            # Check the case after transition
            cursor.execute("SELECT list, status, fy_id, is_finalized FROM cases WHERE id = ?", (case_id,))
            updated_case = cursor.fetchone()
            print(f"After Valid transition: List={updated_case[0]}, Status={updated_case[1]}, FY_ID={updated_case[2]}, Finalized={updated_case[3]}")

            # Verify fy_id is valid
            cursor.execute("SELECT COUNT(*) FROM financial_years WHERE id = ?", (updated_case[2],))
            ...
            if orphaned_count > 0:
                ...
                for case in orphaned_cases:
                    print(f"  ID: {case[0]}, Transaction: {case[1]}, List: {case[2]}, Status: {case[3]}, FY_ID: {case[4]}")
