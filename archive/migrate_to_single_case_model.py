...
def migrate_to_single_case_model():
    ...
    try:
        ...
        for case in cases:
            case_id, transaction_no, status, list_name, lc_recommendation, evidence_path, recovery_path, minutes_path, supporting_path = case
            ...
            assessment_status = status if status in ["Alleged", "Under Assessment", "Valid", "Confirmed"] else "Alleged"
            lc_status = None
            ...
            cursor.execute("""
                UPDATE cases SET
                    ...
                    {evidence_update}
                WHERE id = ?
            """.format(evidence_update=", evidence_paths = ?" if evidence_paths_json else ""),
                ([base_transaction_no, assessment_status, lc_status, ','.join(suffixes)] +
        ...
        try:
            cursor.execute("CREATE INDEX idx_cases_base_transaction_no ON cases(base_transaction_no)")
            cursor.execute("CREATE INDEX idx_cases_assessment_status ON cases(assessment_status)")
            ...
            cursor.execute("CREATE INDEX idx_cases_write_off_group_id ON cases(write_off_group_id)")
...
def verify_migration():
    ...
    try:
        ...
        required_columns = ['base_transaction_no', 'assessment_status', 'lc_status', 'suffixes', 'write_off_group_id', 'evidence_paths']
        ...
        for sample in samples:
            print(f"  Base: {sample[0]}, Assessment: {sample[1]}, LC: {sample[2]}, Suffixes: {sample[3]}, Finalized: {sample[4]}")

        # Check counts by status
        cursor.execute("SELECT assessment_status, COUNT(*) FROM cases GROUP BY assessment_status")
