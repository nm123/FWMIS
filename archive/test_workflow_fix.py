...
def test_workflow_fix():
    ...
    cursor.execute(
        "SELECT transaction_no, list, status, is_finalized FROM cases WHERE status = 'Confirmed'"
    )
    ...
    for case in confirmed_cases:
        ...
        print(
            f"  {transaction_no}: List='{list_name}', Status='{status}', Finalized={is_finalized}"
        )

    # Check cases that would appear in Lead Schedule (list = 'Lead Schedule' OR status = 'Confirmed')
    cursor.execute(
        "SELECT transaction_no, list, status, is_finalized FROM cases WHERE (list = 'Lead Schedule' AND is_finalized = 0) OR status = 'Confirmed'"
    )
