...
class EditCaseDialog(QDialog):
    ...
    def __init__(self, case_data, parent=None, selected_list=None):
        print(
            f"DEBUG: EditCaseDialog.__init__ called with case_data type: {type(case_data)}"
        )
        if hasattr(case_data, "__len__"):
            print(f"DEBUG: case_data length: {len(case_data)}")
            print(
                f"DEBUG: case_data first 5 elements: {case_data[:5] if len(case_data) > 5 else case_data}"
            )
