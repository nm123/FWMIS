...
def delete_case(dialog_instance):
    ...
    display_no = get_display_transaction_no(
    ...
    if reply == QMessageBox.Yes:
        try:
            ...
            QMessageBox.information(
                dialog_instance,
                "Success",
                f"Case {display_no} has been moved to Deleted Cases.",
            )
            ...
            QMessageBox.critical(
                dialog_instance, "Error", f"Failed to delete case: {str(e)}"
            )
...
def open_determination_dialog(dialog_instance):
    ...
    try:
        ...
        QMessageBox.critical(
            dialog_instance, "Error", f"Failed to open determination dialog: {str(e)}"
        )
...
def update_determination_button_visibility(dialog_instance):
    ...
    try:
        ...
        show_determination = (
            selected_assessment_status == "Confirmed"
            and "-LS" in dialog_instance.suffixes
            and (
                not dialog_instance.lc_status
                or dialog_instance.lc_status == "Awaiting LC determination"
            )
        )
