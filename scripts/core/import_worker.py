...
class ImportWorker(QThread):
    ...
    def __init__(self, transactions, category, date_from, date_to, bas_file_path, selected_fy=None):
        super().__init__()
    ...
    def _check_database_integrity(self):
        ...
        try:
            ...
            if orphaned_count > 0:
                print(f"DEBUG: *** DATABASE INTEGRITY WARNING *** Found {orphaned_count} cases with invalid fy_id")
            ...
            cursor.execute("SELECT COUNT(*) FROM financial_years WHERE start_year = ? AND end_year = ?", (start_year, end_year))
            fy_count = cursor.fetchone()[0]

            if fy_count == 0:
                print(f"DEBUG: *** CRITICAL WARNING *** Current FY {fy} not found in database")
    ...
    def _import_transaction(self, transaction):
        ...
        try:
            ...
            print(f"DEBUG: Import - FY: {fy}, start_year: {start_year}, end_year: {end_year}, fy_id: {fy_id}")

            # Check what cases exist for this FY before import
            if fy_id:
                cursor.execute("SELECT COUNT(*), MAX(transaction_no) FROM cases WHERE fy_id = ?", (fy_id,))
                existing_cases = cursor.fetchone()
                print(f"DEBUG: Existing cases in FY {fy} (ID: {fy_id}): {existing_cases[0]} cases, max: {existing_cases[1]}")
            ...
            cursor.execute("SELECT id, start_year, end_year, status FROM financial_years WHERE id = ?", (fy_id,))
            verify_result = cursor.fetchone()
            if not verify_result:
                print(f"DEBUG: *** CRITICAL ERROR *** FY ID {fy_id} does not exist in database!")
                print(f"DEBUG: This indicates a database integrity issue")
                conn.close()
                raise Exception(f"Financial Year ID {fy_id} is invalid. Database integrity check failed. Please contact system administrator.")

            verified_start_year, verified_end_year, verified_status = verify_result[1], verify_result[2], verify_result[3]
            if verified_start_year != start_year or verified_end_year != end_year:
                print(f"DEBUG: *** CRITICAL ERROR *** FY ID {fy_id} mismatch!")
                print(f"DEBUG: Expected: {start_year}-{end_year}, Found: {verified_start_year}-{verified_end_year}")
                conn.close()
                raise Exception(f"Financial Year ID {fy_id} data mismatch. Database integrity check failed. Please contact system administrator.")

            print(f"DEBUG: ✅ FY validation passed - ID: {fy_id}, Status: {verified_status}")
            ...
            if fy_id:
                ...
                print(f"DEBUG: Period lookup for FY {fy_id}, date {date_str}: found period_id = {period_id}")
            ...
            if fy_exists == 0:
                print(f"DEBUG: *** EMERGENCY ERROR *** FY ID {fy_id} disappeared before insert!")
                conn.close()
                raise Exception(f"Critical database error: Financial Year ID {fy_id} no longer exists. Import aborted for safety.")
            ...
            print(f"DEBUG: ImportWorker inserted case {case_number} with ID: {case_id}, fy_id: {fy_id}")

            case_id = cursor.lastrowid
            print(f"DEBUG: Case inserted with ID: {case_id}")

            # Debug: Verify what was actually saved
            cursor.execute("SELECT fy_id, period_id, responsibility_id FROM cases WHERE id = ?", (case_id,))
            ...
            if saved_values:
                ...
                if saved_fy_id_check != fy_id:
                    print(f"DEBUG: *** IMMEDIATE ERROR *** fy_id mismatch right after insert!")
            else:
                print(f"DEBUG: *** ERROR *** Case {case_number} not found by ID after insert!")
            ...
            cursor.execute("SELECT fy_id, id FROM cases WHERE transaction_no = ?", (case_number,))
            validation_result = cursor.fetchone()
            print(f"DEBUG: Validation query result: {validation_result}")

            if not validation_result:
                print(f"DEBUG: *** CRITICAL ERROR *** Case {case_number} not found after insert!")
                conn.rollback()
                conn.close()
                raise Exception(f"Database integrity violation: Case not found after insert. Import aborted.")

            saved_fy_id, saved_case_id = validation_result
            print(f"DEBUG: Case {case_number} (ID: {saved_case_id}) has fy_id: {saved_fy_id}")

            if saved_fy_id != fy_id:
                print(f"DEBUG: *** CRITICAL ERROR *** Case {case_number} saved with wrong fy_id!")
                print(f"DEBUG: Expected: {fy_id}, Saved: {saved_fy_id}")

                # Check if this is an orphaned case issue
                cursor.execute("SELECT start_year, end_year FROM financial_years WHERE id = ?", (saved_fy_id,))
                fy_check = cursor.fetchone()
                if not fy_check:
                    print(f"DEBUG: FY {saved_fy_id} does not exist - this is an orphaned case issue!")
                else:
                    print(f"DEBUG: FY {saved_fy_id} exists: {fy_check[0]}-{fy_check[1]}")

                conn.rollback()
                conn.close()
                raise Exception(f"Database integrity violation: Case saved with incorrect financial year. Import aborted.")
