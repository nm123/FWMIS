...
def create_annexure_sheet(workbook: Workbook, annexure: dict):
    ...
    for row_idx, case in enumerate(annexure['cases'], 7):
        ...
        # Responsibility
        resp_cell = worksheet.cell(row=row_idx, column=2, value=case['responsibility_name'])
        resp_cell.font = data_font
        resp_cell.alignment = data_alignment
        resp_cell.border = thin_border
        
        # Amount
        amount_cell = worksheet.cell(row=row_idx, column=3, value=f"R {case['amount']:,.2f}")
        ...
        # LC Recommendation
        lc_rec_cell = worksheet.cell(row=row_idx, column=5, value="Write-Off Recommended")
        ...
        if lc_minutes_path and os.path.exists(lc_minutes_path):
            ...
            lc_cell.hyperlink = Hyperlink(target=lc_minutes_path, tooltip=lc_minutes_path)
