...
from scripts.Utilities.financial_utils import get_financial_year, get_current_open_financial_year
...
def find_duplicates(transaction, category_name):
    ...
    try:
        ...
        print(f"DEBUG: Transaction details: responsibility='{transaction['responsibility']}', category='{category_name}', amount={abs(transaction['amount']):.2f}")
        ...
        if resp_id:
            ...
            if len(rows) > 0:
                ...
                if not duplicates:
                    print(f"DEBUG: No exact matches found for: resp_id={resp_id}, category='{category_name}', amount={transaction_amount:.2f}, fy_id={fy_id} or orphaned FYs")
