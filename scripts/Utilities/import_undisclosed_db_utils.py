"""
DB utilities for undisclosed imports.
"""
import sqlite3
...
def check_period_status(date_from, date_to):
...
def find_duplicates(transaction, category):
    ...
    try:
        ...
        print(f"DEBUG: Transaction details: responsibility='{transaction['responsibility']}', category='{category['name']}', amount={abs(transaction['amount']):.2f}")
        ...
        if resp_id:
            ...
            print(f"DEBUG: Cases with matching category '{category['name']}': {cat_count}")
        ...
        if resp_id:
            ...
            if len(rows) > 0:
                ...
                for orphaned_fy_id in orphaned_fy_ids:
                    ...
                    cursor.execute("""
                        ...
                        WHERE responsibility_id = ?
                          ...
                          AND fy_id = ?
                          AND list != 'Deleted Cases'
                    """, (resp_id, category['name'], transaction_amount, orphaned_fy_id))
                ...
                if not duplicates:
                    print(f"DEBUG: No exact matches found for: resp_id={resp_id}, category='{category['name']}', amount={transaction_amount:.2f}, fy_id={fy_id} or orphaned FYs")
