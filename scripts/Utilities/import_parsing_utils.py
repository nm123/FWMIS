            ...
            for pattern in patterns:
                ...
                if match:
                    try:
                        ...
                        extracted_date_from = datetime.strptime(date_from_str, '%d/%m/%Y').date()
                        extracted_date_to = datetime.strptime(date_to_str, '%d/%m/%Y').date()
                        break
                    except ValueError:
                        continue
        ...
        if not extracted_date_from:
            ...
            if len(found_dates) >= 2:
                try:
                    ...
                    extracted_date_from = datetime.strptime(found_dates[0], '%d/%m/%Y').date()
                    extracted_date_to = datetime.strptime(found_dates[1], '%d/%m/%Y').date()
                except ValueError:
                    pass
