...
def add_annexure_content(
    story, annexure, title_style, subtitle_style, styles, detailed_export=False
):
    ...
    if detailed_export:
        ...
        table_data = [
            ["Case No", "Responsibility", "Amount", "Description", "LC Recommendation"]
        ]
    ...
    if detailed_export:
        table_data.append(
            ["", "", "", "TOTAL:", f"R {annexure['total_amount']:,.2f}", ""]
        )
    else:
    ...
    if detailed_export:
        ...
        table = Table(
            table_data,
            colWidths=[1.2 * inch, 1.5 * inch, 1 * inch, 3 * inch, 1.5 * inch],
        )
    table.setStyle(
        TableStyle(
            [
...
def add_lc_minutes_with_indexing(story, annexure, evidence_index):
    ...
    for doc_path, info in evidence_index.items():
        if info["cases"]:  # Only include docs that are used
            # Document header
            doc_title = Paragraph(
                f"Document: {info['filename']}", getSampleStyleSheet()["Heading3"]
            )
            ...
            # Page reference
            page_range = (
                f"Pages {current_page} - {current_page + info['page_count'] - 1}"
            )
            page_para = Paragraph(
                f"Location: {page_range}", getSampleStyleSheet()["Italic"]
            )
...
def add_lc_minutes_content(story, annexure):
    ...
    for case in annexure["cases"]:
        ...
        if lc_minutes_path and os.path.exists(lc_minutes_path):
            # Case header
            case_header = Paragraph(
                f"Case: {case['transaction_no']}", getSampleStyleSheet()["Heading3"]
            )
            ...
            if lc_minutes_path.lower().endswith(".pdf"):
                ...
                file_msg = Paragraph(
                    f"LC Minutes file: {lc_minutes_path}",
                    getSampleStyleSheet()["Normal"],
                )
            ...
            no_minutes = Paragraph(
                f"Case {case['transaction_no']}: LC Minutes not available",
                getSampleStyleSheet()["Normal"],
            )
...
def embed_pdf_minutes(story, pdf_path):
    ...
    try:
        ...
        minutes_ref = Paragraph(
            f"LC Minutes embedded (see attached PDF)", getSampleStyleSheet()["Normal"]
        )
        ...
        fallback_msg = Paragraph(
            f"LC Minutes available at: {pdf_path}", getSampleStyleSheet()["Normal"]
        )
