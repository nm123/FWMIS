...
class StreamingExcelExporter:
    ...
    def export_cases_to_excel_streaming(
        self, cases: Iterator[Dict], filepath: str, sheet_name: str = "Cases"
    ) -> str:
    ...
    def _write_chunk_to_worksheet(
        self, ws, chunk: List[Dict], start_row: int, headers: List[str]
    ):
    ...
    def _add_summary_to_worksheet(
        self, ws, case_count: int, total_amount: float, num_columns: int
    ):
        ...
        for col in range(1, len(headers) + 1):
            ...
            for row in ws[column_letter]:
                try:
                    ...
                    logging.debug(f"Failed to process row value: {e}")
                    pass
...
def create_optimized_excel_export(dialog, cases_data: List[Dict], filename: str) -> str:
    ...
    try:
        ...
        return exporter.export_cases_to_excel_streaming(
            cases_iterator(), filepath, "Cases Export"
        )
...
def validate_memory_for_export(cases_count: int) -> bool:
    ...
    try:
        ...
        if (
            estimated_memory_mb > available_memory_gb * 1024 * 0.5
        ):  # Use max 50% of available memory
            logger.warning(
                f"Large export detected: {cases_count} cases, {estimated_memory_mb:.1f}MB estimated"
            )
            return False
