...
def update_database_and_workflow(dialog_instance, case: dict) -> bool:
    ...
    try:
        ...
        """, (
            ...
            case["base_transaction_no"], evidence_paths_json, case["evidence_path"], assessment_status_text, lc_status_text,
        ...
        if selected_assessment_status in ["Valid", "Confirmed"]:
            if not handle_case_status_change(dialog_instance.case_id, dialog_instance.base_transaction_no, selected_assessment_status):
        ...
        selected_lc_status = dialog_instance.lc_status_combo.currentText() if dialog_instance.lc_status_combo.isVisible() else None
        if selected_lc_status and selected_lc_status != "Awaiting LC determination":
            if not handle_loss_control_status_change(dialog_instance.case_id, dialog_instance.base_transaction_no, selected_lc_status):
                QMessageBox.warning(dialog_instance, "Warning", f"Case saved but LC workflow status update failed for {selected_lc_status}")
            else:
                print(f"LOG: Updated LC workflow status to {selected_lc_status} for case {dialog_instance.base_transaction_no}")
