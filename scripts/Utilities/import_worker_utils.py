...
def import_cases(dialog):
    ...
    transactions_to_import = [t for t in dialog.transactions if not t.get('marked_for_removal', False)]

    if not transactions_to_import:
        QMessageBox.warning(dialog, "No Transactions", "All transactions have been marked for removal. Nothing to import.")
        return
...
def perform_import(dialog):
    ...
    print(f"DEBUG: Starting import with {len(transactions_to_import)} transactions (filtered from {len(dialog.transactions)})")

    dialog.progress_bar.setVisible(True)
    dialog.progress_bar.setValue(0)
    dialog.import_button.setEnabled(False)

    dialog.worker = ImportWorker(transactions_to_import, dialog.category,
                            dialog.date_from, dialog.date_to, dialog.bas_file_path)
