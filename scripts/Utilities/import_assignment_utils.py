...
def assign_case_numbers(dialog):
    ...
    if not dialog.transactions:
        QMessageBox.warning(dialog, "No Transactions", "No transactions to assign case numbers to")
        return
    ...
    try:
        ...
        transactions_to_assign = [t for t in dialog.transactions if not t.get('marked_for_removal', False)]
        ...
        QMessageBox.critical(dialog, "Error", f"Failed to assign case numbers:\n{str(e)}")