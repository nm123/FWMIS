...
class DeterminationDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.determined_amount_edit.setPlaceholderText("Enter exact F&W amount determined")
        if self.case_data[11]:  # Pre-populate with current amount
            self.determined_amount_edit.setText(str(self.case_data[11]))
        determination_layout.addRow("Exact Amount Determined (R):", self.determined_amount_edit)
        ...
        self.criminal_charges_cb = QCheckBox("Criminal Charges must be laid against the person responsible")
        self.disciplinary_cb = QCheckBox("Disciplinary action needs to be taken against the culprit")
        self.loss_recovery_cb = QCheckBox("The loss must be recovered from the person responsible")
        ...
        self.notes_edit.setPlaceholderText("Enter any additional notes from the determination meeting...")
    ...
    def add_committee_member(self):
        ...
        if member_name:
            if member_name not in self.committee_members:
                ...
                QMessageBox.warning(self, "Duplicate Member", "This member is already in the list.")
        else:
    ...
    def save_determination(self):
        try:
            ...
            if not determined_amount_text:
                QMessageBox.warning(self, "Invalid Input", "Please enter the determined amount.")
                return
            ...
            try:
                ...
                QMessageBox.warning(self, "Invalid Input", "Determined amount must be a positive number.")
                return
            ...
            if not any(recommendations.values()):
                QMessageBox.warning(self, "Invalid Input", "Please select at least one recommendation.")
                return

            if not self.committee_members:
                QMessageBox.warning(self, "Invalid Input", "Please add at least one committee member.")
                return

            # Convert date
            determination_date = self.determination_date_edit.date().toString("yyyy-MM-dd")
            ...
            # Log audit trail
            save_audit_log("determination_made", {
            ...
            QMessageBox.information(self, "Success", "Determination saved successfully!")
            self.accept()

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to save determination: {str(e)}")