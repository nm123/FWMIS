...
class WriteOffManagementDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.submissions_table.setHorizontalHeaderLabels([
            "Submission ID", "Created Date", "Status", "Cases", "Total Amount", "Actions", "Details"
        ])
    ...
    def load_submissions(self):
        ...
        try:
            ...
            for row, submission in enumerate(submissions):
                sub_id, submission_id, created_date, status, case_ids_json, total_amount, notes = submission
                ...
                try:
                    ...
                    self.submissions_table.setItem(row, 3, QTableWidgetItem(str(len(case_ids))))
                ...
                details_btn.clicked.connect(lambda checked, s_id=sub_id: self.view_submission_details(s_id))
    ...
    def create_actions_widget(self, submission_id, submission_code, status):
        ...
        if status == "Draft":
            ...
            submit_btn.clicked.connect(lambda: self.submit_submission(submission_id, submission_code))
            layout.addWidget(submit_btn)

        elif status == "Submitted":
            approve_btn = QPushButton("Approve")
            approve_btn.clicked.connect(lambda: self.approve_submission(submission_id, submission_code))
            approve_btn.setStyleSheet("QPushButton { background-color: #4CAF50; color: white; }")
            layout.addWidget(approve_btn)

            reject_btn = QPushButton("Reject")
            reject_btn.clicked.connect(lambda: self.reject_submission(submission_id, submission_code))
            reject_btn.setStyleSheet("QPushButton { background-color: #f44336; color: white; }")
    ...
    def submit_submission(self, submission_id, submission_code):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                # Log audit trail
                save_audit_log("write_off_submission_submitted", {
                    "timestamp": datetime.now().isoformat(),
                    "submission_id": submission_code,
                    "submission_db_id": submission_id
                }, self.fy)

                QMessageBox.information(self, "Success", f"Submission {submission_code} has been submitted for approval.")
                self.load_submissions()

            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to submit submission: {str(e)}")
            finally:
                conn.close()
    ...
    def approve_submission(self, submission_id, submission_code):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                cursor.execute("SELECT case_ids FROM write_off_submissions WHERE id = ?", (submission_id,))
                    ...
                    try:
                        ...
                        for case_id in case_ids:
                            cursor.execute("""
                                ...
                                WHERE id = ?
                            """, (datetime.now().strftime("%Y-%m-%d"), "Write-off approved", case_id))

                        conn.commit()

                        # Log audit trail
                        save_audit_log("write_off_submission_approved", {
                            "timestamp": datetime.now().isoformat(),
                            "submission_id": submission_code,
                            "submission_db_id": submission_id,
                        ...
                        QMessageBox.critical(self, "Error", "Invalid case IDs in submission.")
    ...
    def reject_submission(self, submission_id, submission_code):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                # Log audit trail
                save_audit_log("write_off_submission_rejected", {
                    "timestamp": datetime.now().isoformat(),
                    "submission_id": submission_code,
                    "submission_db_id": submission_id
                }, self.fy)

                QMessageBox.information(self, "Success", f"Submission {submission_code} has been rejected.")
                self.load_submissions()

            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to reject submission: {str(e)}")
            finally:
                conn.close()
    ...
    def view_submission_details(self, submission_id):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to load submission details: {str(e)}")
        finally:
            conn.close()