...
class EditCaseLogic:
    ...
    def load_case_data(self):
        ...
        # Set responsibility
        resp = next((r for r in self.dialog.responsibilities if r["id"] == self.case_data[10]), None)
        if resp:
            self.dialog.responsibility_edit.setText(resp["name"])

        # Set dates
        if self.case_data[2]:  # date_incurred
            self.dialog.date_incurred_edit.setDate(QDate.fromString(self.case_data[2], "yyyy-MM-dd"))
        if self.case_data[3]:  # date_identified
            self.dialog.date_identified_edit.setDate(QDate.fromString(self.case_data[3], "yyyy-MM-dd"))
        if self.case_data[4]:  # date_reported
            self.dialog.date_reported_edit.setDate(QDate.fromString(self.case_data[4], "yyyy-MM-dd"))
        ...
        if self.case_data[7]:  # bas_payment_date
            self.dialog.bas_payment_date_edit.setDate(QDate.fromString(self.case_data[7], "yyyy-MM-dd"))
        ...
        if len(self.case_data) > 19 and self.case_data[19]:  # assessment_date
            self.dialog.assessment_date_edit.setDate(QDate.fromString(self.case_data[19], "yyyy-MM-dd"))
    ...
    def save_case(self):
        ...
        try:
            ...
            category = next((c for c in self.dialog.categories if c["name"] == category_name), None)
            if category:
            ...
            if missing_fields:
                QMessageBox.warning(self.dialog, "Invalid Input", f"The following fields are required: {', '.join(missing_fields)}")
                return
            ...
            try:
                ...
                QMessageBox.warning(self.dialog, "Invalid Input", "Amount must be a positive number.")
                return

            if not self.dialog.selected_responsibility_id:
                QMessageBox.warning(self.dialog, "Invalid Input", "Please select a responsibility.")
                return

            # Validate supporting evidence if compulsory
            if self.dialog.supporting_evidence_compulsory and not self.dialog.evidence_edit.text().strip():
                QMessageBox.warning(self.dialog, "Supporting Evidence Required",
                                  "Supporting Evidence is compulsory for Valid status cases.\n\n"
                                  "Please select a file before saving.")
                return

            # Convert dates
            date_incurred_str = self.dialog.date_incurred_edit.date().toString("yyyy-MM-dd")
            date_identified_str = self.dialog.date_identified_edit.date().toString("yyyy-MM-dd")
            date_reported_str = self.dialog.date_reported_edit.date().toString("yyyy-MM-dd")
            bas_payment_date_str = self.dialog.bas_payment_date_edit.date().toString("yyyy-MM-dd")
            assessment_date_str = self.dialog.assessment_date_edit.date().toString("yyyy-MM-dd")
            ...
            existing_period_id = self.case_data[22] if len(self.case_data) > 22 else None
                ...
                if case[field] and case[field].strip():
                    ...
                    if os.path.exists(source_path):
                        ...
                        try:
                            ...
                            QMessageBox.warning(self.dialog, "File Save Error",
                                              f"Failed to save {field} file: {str(e)}")
                            return
                    else:
                        QMessageBox.warning(self.dialog, "File Not Found",
            ...
            QMessageBox.information(self.dialog, "Success", "Case updated successfully.")
            self.dialog.accept()
    ...
    def delete_case(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.information(self.dialog, "Success", f"Case {self.transaction_no} has been moved to Deleted Cases.")
                self.dialog.accept()

            except Exception as e:
                QMessageBox.critical(self.dialog, "Error", f"Failed to delete case: {str(e)}")