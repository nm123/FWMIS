...
class AddCaseLogic:
    ...
    def on_status_changed(self, status):
        ...
        if status == "Valid":
            ...
            if reply == QMessageBox.Yes:
                ...
                self.dialog.file_path_edit.setPlaceholderText("Assessment Evidence is REQUIRED - Select file...")
            else:
    ...
    def update_conditional_fields(self):
        ...
        current_status = self.dialog.status_combo.currentText() if self.dialog.status_combo.count() > 0 else "Alleged"
        ...
        if selected_list == "Lead Schedule":
            ...
            self.dialog.status_combo.addItems(["Alleged", "Under Assessment", "Valid", "Confirmed", "Recovered", "Write Off Recommended"])
        else:
            # For Checklist and other lists, show standard statuses
            self.dialog.status_combo.addItems(["Alleged", "Under Assessment", "Valid", "Confirmed"])

        # Try to restore the previous selection if it's still valid
        if current_status in [self.dialog.status_combo.itemText(i) for i in range(self.dialog.status_combo.count())]:
        ...
        selected_category = self.dialog.category_combo.currentText() if self.dialog.category_combo.count() > 0 else ""
        if selected_category:
            category = next((c for c in self.dialog.categories if c["name"] == selected_category), None)
            if category:
        ...
        self.dialog.bas_date_label.setText("BAS Payment Date:" + (" *" if bas_comp else ""))
        ...
        self.dialog.bas_journal_label.setText("BAS Journal No:" + (" *" if bas_comp else ""))
        self.dialog.bas_journal_label.setVisible(bas_comp)
        self.dialog.bas_journal_no_edit.setVisible(bas_comp)
        self.dialog.bas_journal_date_label.setText("BAS Journal Date:" + (" *" if bas_comp else ""))
    ...
    def save_case(self):
        from scripts.Utilities.add_case_utils import validate_add_data, get_case_data, handle_file_operations
        ...
        try:
            ...
            if fy_id is None:
                QMessageBox.critical(self.dialog, "Financial Year Error",
            ...
            if not handle_file_operations(case, self.dialog.fy, self.dialog.transaction_no):
                return
            ...
            """, (
                case["transaction_no"], case["transaction_no"], case["date_incurred"], case["date_identified"], case["date_reported"],
                case["description"], case["bas_payment_no"], case["bas_payment_date"], case["persal_no"], case["category"],
                case["responsibility_id"], case["amount"], case["source_document"], case["minutes"], case["evidence_path"],
                case["status"], case["list"], case["assessment_assessed_by"], case["assessment_date"], case["assessment_result"],
                case["fy_id"], case["period_id"], 'N/A', 'N/A', 'N/A',
                case["prevention_steps"], case["original_list"], case["attachments"], None, case["bas_journal_no"], case["bas_journal_date"]
            ))
            ...
            if not case["supporting_evidence_path"]:
                ...
                conn.execute("UPDATE cases SET status = ? WHERE transaction_no = ?", ("Missing Supporting Evidence", self.dialog.transaction_no))
                conn.commit()
                QMessageBox.warning(self.dialog, "Supporting Evidence Missing",
                ...
                QMessageBox.information(self.dialog, "Success", "Case added successfully.")
    ...
    def next_case(self):
        ...
        if "Checklist" in [l["name"] for l in self.dialog.lists if l.get("is_system", False) and l["name"] != "Deleted Cases"]:
    ...
    def send_reminder_email(self):
        if not self.dialog.selected_responsibility_id:
            QMessageBox.warning(self.dialog, "No Responsibility", "Please select a responsibility first.")
            return
        ...
        if not emails:
            QMessageBox.warning(self.dialog, "No Contacts", "No valid email contacts found for this responsibility.")
            return
        ...
        mail.To = ";".join(emails)
        mail.Subject = f"Reminder: Assessment Evidence for Case {self.dialog.transaction_no}"
        mail.Body = body
        mail.Display()

    def open_assessment(self):
        if not self.dialog.evidence_edit.text():
            QMessageBox.warning(self.dialog, "No Evidence", "Please upload evidence before assessment.")
            return
        ...
        if dialog.exec_():
            ...
            cursor.execute("UPDATE cases SET assessment_assessed_by = ?, assessment_date = ?, assessment_result = ?, status = ?, list = ? WHERE transaction_no = ?",
                            (assessment_data["assessed_by"], assessment_data["assessment_date"], assessment_data["result"], assessment_data["result"], "Lead Schedule", self.dialog.transaction_no))
            conn.commit()
            conn.close()
