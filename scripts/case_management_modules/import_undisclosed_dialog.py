...
class ImportUndisclosedCasesDialog(QDialog):
    ...
    def parse_file(self):
        ...
        try:
            ...
            self.transactions = self.parser.parse_file(
                self.bas_file_path, self.date_from, self.date_to
            )
            ...
            QMessageBox.critical(
                self, "Parse Error", f"Failed to parse BAS file:\n{str(e)}"
            )
    ...
    def populate_transactions_table(self):
        ...
        for i, transaction in enumerate(self.transactions):
            ...
            resp_item.setForeground(
                QColor.blue
            )  # Make it blue to indicate it's clickable
            font = resp_item.font()
            ...
            self.transactions_table.setItem(
                row, 1, QTableWidgetItem(transaction["type"])
            )
            ...
            if hasattr(self.logic, "duplicate_check_results") and i < len(
                self.logic.duplicate_check_results
            ):
            ...
            view_btn.clicked.connect(
                lambda checked, trans=transaction: self.view_transaction_details(trans)
            )
            ...
            if has_duplicates:
                ...
                compare_btn.clicked.connect(
                    lambda checked, trans=transaction, dups=result[
                        "duplicates"
                    ]: self.compare_duplicates(trans, dups)
                )
    ...
    def compare_duplicates(self, transaction, duplicates):
        ...
        transaction_copy["category_name"] = (
            self.category["name"] if self.category else "N/A"
        )
    ...
    def import_finished(self, imported_cases):
        ...
        if imported_cases:
            try:
                ...
                print(
                    f"DEBUG: import_finished - imported_cases: {imported_cases[:3]}..."
                )  # Show first 3
                ...
                max_imported = max(
                    int(case.split(str(fy_end_year))[1]) for case in imported_cases
                )
    ...
    def import_error(self, error_msg):
        ...
        QMessageBox.critical(
            self, "Import Error", f"Failed to import cases:\n{error_msg}"
        )
