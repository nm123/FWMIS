...
from scripts.ui.dialogs.edit_case.edit_case_handlers import (
    browse_evidence,
    browse_minutes,
    browse_source_doc,
    on_cancel_clicked,
...
from .edit_case_ui import NoWheelComboBox, setup_edit_ui
...
class EditCaseDialog(QDialog):
    def __init__(self, case_data, parent=None):
        ...
        try:
            ...
            # Connect signals
            self.select_responsibility_button.clicked.connect(
                lambda: select_responsibility(self)
            )
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to initialize Edit Case dialog: {str(e)}"
            )
            self.reject()
