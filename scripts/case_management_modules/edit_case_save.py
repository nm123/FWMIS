...
def save_case_components(dialog_instance):
    ...
    try:
        # Check if case is finalized
            if len(dialog_instance.case_data) > 37 and dialog_instance.case_data[37]:  # is_finalized
                QMessageBox.warning(dialog_instance, "Case Finalized",
            ...
            category = next((c for c in dialog_instance.categories if c["name"] == category_name), None)
            if category:
            ...
            if bas_validation_errors and not any([bas_comp, persal_comp]):
                ...
                QMessageBox.warning(dialog_instance, "Invalid Input", f"The following fields are required: {', '.join(missing_fields)}")
                return
            try:
                ...
                QMessageBox.warning(dialog_instance, "Invalid Input", "Amount must be a positive number.")
                return
            ...
            if not dialog_instance.selected_responsibility_id:
                ...
                if isinstance(dialog_instance.case_data, dict):
                    dialog_instance.selected_responsibility_id = dialog_instance.case_data.get('responsibility_id')
                elif len(dialog_instance.case_data) > 10:
                    dialog_instance.selected_responsibility_id = dialog_instance.case_data[10]
            if not dialog_instance.selected_responsibility_id:
                QMessageBox.warning(dialog_instance, "Invalid Input", "Please select a responsibility.")
                return
            # Validate Loss Control fields
            loss_control_status = dialog_instance.lc_status_combo.currentText()
            if loss_control_status == "Recovered" and not dialog_instance.recovery_evidence_edit.text().strip():
                QMessageBox.warning(dialog_instance, "Recovery Evidence Required",
            ...
            if loss_control_status in ["Recovered", "Write Off Recommended"] and not dialog_instance.minutes_edit.text().strip():
                QMessageBox.warning(dialog_instance, "Loss Control Minutes Required",
            ...
            if selected_assessment_status in ["Valid", "Confirmed"]:
                ...
                if not current_evidence_path or not os.path.exists(current_evidence_path):
                    QMessageBox.warning(dialog_instance, "Cannot Save",
                    ...
                    print(f"LOG: Blocked save for case {dialog_instance.base_transaction_no} due to missing assessment evidence")
                    return
                print(f"LOG: Found assessment evidence for case {dialog_instance.base_transaction_no}: {current_evidence_path}")
            ...
            if selected_lc_status in ["Recovered", "Write Off Recommended"]:
                if selected_lc_status == "Recovered" and not dialog_instance.recovery_evidence_edit.text().strip():
                    QMessageBox.warning(dialog_instance, "Recovery Evidence Required",
            ...
            existing_fy_id = dialog_instance.case_data[21] if len(dialog_instance.case_data) > 21 else None  # fy_id
            existing_period_id = dialog_instance.case_data[22] if len(dialog_instance.case_data) > 22 else None  # period_id
            ...
            try:
                dialog_instance.category_combo.currentTextChanged.disconnect(dialog_instance.schedule_update_conditional_fields)
            ...
            case_folder = os.path.join(supporting_evidence_folder, f"Case {dialog_instance.base_transaction_no}")
                ...
                try:
                    ...
                    if os.path.exists(dest_path):
                        try:
                            ...
                            QMessageBox.critical(dialog_instance, "File Permission Error",
                            ...
                            dialog_instance.category_combo.currentTextChanged.connect(dialog_instance.schedule_update_conditional_fields)
                    ...
                    QMessageBox.critical(dialog_instance, "File Copy Permission Error",
                    ...
                    QMessageBox.critical(dialog_instance, "File System Error",
                    ...
                    QMessageBox.critical(dialog_instance, "File Copy Error",
            ...
            print(f"LOG: Uploaded evidence for case {dialog_instance.base_transaction_no} in {upload_time:.2f}s")
            ...
            try:
                ...
                print(f"LOG: Started transaction for case {dialog_instance.base_transaction_no} save")
                ...
                """, (
                    ...
                    case["base_transaction_no"], evidence_paths_json, assessment_status_text, lc_status_text,
                    case["criminal_charges"], case["disciplinary_process"], case["loss_recovery"],
                    case["prevention_steps"],
                    dialog_instance.case_id
                ))

                print(f"LOG: Saved case {dialog_instance.base_transaction_no} with assessment_status='{assessment_status_text}', evidence_paths updated")

                conn.commit()
                print(f"LOG: Committed transaction for case {dialog_instance.base_transaction_no} save")
                ...
                try:
                    ...
                    selected_assessment_status = dialog_instance.assessment_status_combo.currentText()
                    if selected_assessment_status in ["Valid", "Confirmed"]:
                        if not dialog_instance.handle_case_status_change(dialog_instance.case_id, dialog_instance.base_transaction_no, selected_assessment_status):
                                QMessageBox.warning(dialog_instance, "Warning", f"Case saved but workflow status update failed for {selected_assessment_status}")
                        else:
                            print(f"LOG: Updated workflow status to {selected_assessment_status} for case {dialog_instance.base_transaction_no}")

                    QMessageBox.information(dialog_instance, "Success", "Case updated successfully.")
                ...
                QMessageBox.critical(dialog_instance, "Database Error", f"Failed to save case to database: {str(e)}")
                return
    except Exception as e:
