...
class ImportLogic:
    ...
    def perform_import(self):
        ...
        transactions_to_import = [
            t
            for t in self.dialog.transactions
            if not t.get("marked_for_removal", False)
        ]
        ...
        print(
            f"DEBUG: Starting import with {len(transactions_to_import)} transactions (filtered from {len(self.dialog.transactions)})"
        )
        ...
        self.dialog.worker = ImportWorker(
            transactions_to_import,
            self.dialog.category,
            self.dialog.date_from,
            self.dialog.date_to,
            self.dialog.bas_file_path,
            self.dialog.selected_fy,
        )
    ...
    def assign_case_numbers(self):
        ...
        if not self.dialog.transactions:
            QMessageBox.warning(
                self.dialog,
                "No Transactions",
                "No transactions to assign case numbers to",
            )
            return
        ...
        try:
            ...
            print(
                f"DEBUG: assign_case_numbers - max_existing: {max_existing}, current_counter: {current_counter}"
            )
            ...
            print(
                f"DEBUG: All cases for FY {fy}: count={all_cases[0]}, max={all_cases[1]}, min={all_cases[2]}"
            )
            ...
            self.dialog.next_counter_value = current_counter + len(
                transactions_to_assign
            )
            ...
            assigned_numbers = [
                t.get("case_number", "No number") for t in transactions_to_assign[:5]
            ]
            ...
            QMessageBox.critical(
                self.dialog, "Error", f"Failed to assign case numbers:\n{str(e)}"
            )
    ...
    def import_cases(self):
        if not self.dialog.transactions:
            QMessageBox.warning(
                self.dialog, "No Transactions", "No transactions to import"
            )
            return
        ...
        if period_status["status"] == "closed":
            ...
            if current_period:
                ...
                start_date = datetime.strptime(
                    current_period["start_date"], "%Y-%m-%d"
                ).date()
                end_date = datetime.strptime(
                    current_period["end_date"], "%Y-%m-%d"
                ).date()

                reply = QMessageBox.question(
                    self.dialog,
                    "Closed Period - Action Required",
