...
def test_case_creation_and_editing():
    ...
    try:
        ...
        cursor.execute("SELECT id, start_year, end_year, status FROM financial_years ORDER BY id")
        ...
        if test_case:
            ...
            print(f"Testing workflow transition for case {transaction_no} (ID: {case_id})")
            ...
            # Reset for next test
            cursor.execute("UPDATE cases SET status = ?, is_finalized = 0 WHERE id = ?", ("Alleged", case_id))
            conn.commit()
            ...
            updated_case = cursor.fetchone()
            print(f"After Confirmed transition: List={updated_case[0]}, Status={updated_case[1]}, FY_ID={updated_case[2]}")
        else:
